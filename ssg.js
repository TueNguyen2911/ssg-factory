...
class SSG {
  ...
  createIndexHtmlFile = () => {
    return new Promise((resolve, reject) => {
      const links = this.generatedFiles
        .map((param, index) => {
      ...
      let write = async () => {
        this.writeHTMLFile(path.join(this.outputPath, "index.html"), html);
      };
      write()
        .then(() => resolve)
        .catch(() => reject());
  ...
  processInput = async (filepath) => {
    ...
    fs.readdirSync(this.outputPath).forEach((file) => {
      ...
      if (
        this.filePaths.indexOf(outputFolderFile) < 0 &&
        path.extname(outputFolderFile) == "html"
      ) {
    ...
    await Promise.all(
      this.filePaths.map(async (file) => {
        await this.createHTMLFile(file, path.extname(file));
      })
    );
    await Promise.all(
      this.toBeGenerated.map(async (param) => {
